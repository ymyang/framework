...
//        System.err.println("表名转换成Java类名0: " + tableName);
        String singularize = inflectorUtil.singularize(tableName);
//        System.err.println("表名转换成Java类名1: " + singularize);
        String javaClassName = columnToJava(singularize);
...
//        String projectPath = System.getProperty("user.dir") + File.separator + "src" + File.separator;
        ...
        if (template.contains("Controller.java.vm") && StringUtils.isNotBlank(config.getControllerPath()) && StringUtils.isNotBlank(config.getControllerPath())) {
            String path = config.getControllerPath() + javaPath
                    + config.getControllerPackageName().replace(".", File.separator)
                    + File.separator + "back" + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + "Back" + className + "Controller.java";
        }

        if (template.contains("QueryParam.java.vm") && StringUtils.isNotBlank(config.getEntityPath()) && StringUtils.isNotBlank(config.getQueryParamPackageName())) {
            String path = config.getQueryParamPath() + javaPath
                    + config.getQueryParamPackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "QueryParam.java";
        }

        if (template.contains("UpdateParam.java.vm") && StringUtils.isNotBlank(config.getUpdateParamPath()) && StringUtils.isNotBlank(config.getUpdateParamPackageName())) {
            String path = config.getUpdateParamPath() + javaPath
                    + config.getUpdateParamPackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "UpdateParam.java";
        }

        if (template.contains("CreateParam.java.vm") && StringUtils.isNotBlank(config.getCreateParamPath()) && StringUtils.isNotBlank(config.getCreateParamPackageName())) {
            String path = config.getCreateParamPath() + javaPath
                    + config.getCreateParamPackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "CreateParam.java";
        }

        if (template.contains("Output.java.vm") && StringUtils.isNotBlank(config.getDtoPath()) && StringUtils.isNotBlank(config.getDtoPackageName())) {
            String path = config.getDtoPath() + javaPath
                    + config.getDtoPackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "Dto.java";
        }

        if (template.contains("Entity.java.vm") && StringUtils.isNotBlank(config.getEntityPath()) && StringUtils.isNotBlank(config.getEntityPackageName())) {
            String path = config.getEntityPath() + javaPath
                    + config.getEntityPackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "Entity.java";
        }

        if (template.contains("Service.java.vm") && StringUtils.isNotBlank(config.getServicePath()) && StringUtils.isNotBlank(config.getServicePackageName())) {
            String path = config.getServicePath() + javaPath
                    + config.getServicePackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "Service.java";
        }

        if (template.contains("ServiceImpl.java.vm") && StringUtils.isNotBlank(config.getServicePath()) && StringUtils.isNotBlank(config.getServicePackageName())) {
            String path = config.getServicePath() + javaPath
                    + config.getServicePackageName().replace(".", File.separator)
        ...
        if (template.contains("Mapper.java.vm") && StringUtils.isNotBlank(config.getMapperPath()) && StringUtils.isNotBlank(config.getMapperPackageName())) {
            String path = config.getMapperPath() + javaPath
                    + config.getMapperPackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "Mapper.java";
        }
        if (template.contains("Mapper.xml.vm") && StringUtils.isNotBlank(config.getMapperPath()) && StringUtils.isNotBlank(config.getMapperPackageName())) {
        ...
        if (template.contains("FeignClient.java.vm") && StringUtils.isNotBlank(config.getFeignClientPath()) && StringUtils.isNotBlank(config.getFeignClientPackageName())) {
            String path = config.getFeignClientPath() + javaPath
                    + config.getFeignClientPackageName().replace(".", File.separator)
                    + File.separator + moduleName + File.separator;
            new File(path).mkdirs();
            return path + className + "FeignClient.java";
        }
        if (template.contains("FeignFallbackFactory.java.vm") && StringUtils.isNotBlank(config.getFeignClientPath()) && StringUtils.isNotBlank(config.getFeignClientPackageName())) {
            String path = config.getFeignClientPath() + javaPath
                    + config.getFeignClientPackageName().replace(".", File.separator)
